...
public class BuildProjectEnquiry2 {
	...
	public void ProjectEnquiry() throws Exception {
		...
		//Data Driver Input from properties file
		...
		nameField.sendKeys(properties.getProperty("name"));
		emailField.sendKeys(properties.getProperty("email"));
		phoneField.sendKeys(properties.getProperty("phonenumber"));
		...
		for(WebElement listItem: scroll) {
			if(listItem.getText().equals(properties.getProperty("selectedProject"))) {
